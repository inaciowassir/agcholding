...
class SPagination
{
    ...
    private static function a($page, $content, array $classes = [])
    {
        ...
        if(!is_null($page))
        {
            self::$anchors[] = sprintf('<a href="' . $url. '%d" class="%s" data-page="%d">%s</a>', $page, implode(" ", $classes), $page, $content);
        }else
        {
            self::$anchors[] = sprintf('<a href="javascript:void(0)" class="%s" data-page="%d">%s</a>',  implode(" ", $classes), $page, $content);
        }
