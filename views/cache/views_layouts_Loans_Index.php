        ...
        <div class="col-lg-12">
          <div class="card">
			...
			<?php foreach($loans as $key => $loan): ?>
			  <tr>
				<td><?php echo $employer['fullName'] ?></td>
				<td><?php echo number_format($loan['monthlyInstallment'], 2) ?></td>
				<td><?php echo $loan['loanTerms'] ?> meses</td>
				<td><?php echo number_format($loan['effortRate'], 2) ?> %</td>
				<td><?php echo number_format($loan['requestedAmount'], 2) ?></td>
				<td><?php echo number_format($loan['totalDisbursement'], 2) ?></td>
				<td>
				    <a href="<?php echo route('loans/details/'. $key) ?>" class="btn btn-primary btn-small"><i class="bi bi-eye"></i></a>
				</td>
			  </tr>
			<?php endforeach; ?>
                    </tbody>
